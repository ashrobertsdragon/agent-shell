"""Shared system-prompt construction for every LLM backend.

Kept in its own leaf module (rather than agent/__init__.py) so it has
no dependency on the package's own __init__ and can be imported by
backend submodules without risking a circular import if __init__.py
later needs to import those submodules itself (e.g. for a backend
registry).
"""
